...
`case/waveshare-raspberry-pi-pico.stl`: both halves in one file, PLA, 0.2 mm layers, no supports.
Snaps together, micro-USB slot on the end, screen window, the four buttons and the joystick poke
through. The design is Tomáš Plass's; the STL is generated by `tools/step` from a parametric
recreation in `case/waveshare-raspberry-pi-pico/` (`case/README.md` says how to change it). Keycaps and a
joystick dome are in `case/out/` (PLA, 0.12 mm layers). `case/gen.py` regenerates them if your
parts differ.
...
MIT for everything here except where noted: the v0 case is Tomáš Plass's design, CC BY-NC 4.0, both
his mesh in `reference/plass/` and the STEP recreation and its outputs in `case/`;
