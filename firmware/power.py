...
# wallet can show the cell voltage. Everything here is optional: with nothing wired, vbat() is 0.0
# and the home screen just shows USB.
#
...
def vbat():
    """Cell voltage at the divider, volts. 0.0 when nothing is wired or the switch is off."""
    _init()
    try:
...
def status():
    v = vbat()
    return {"usb": usb(), "vbat": round(v, 2), "percent": percent(v), "low": 1.0 <= v < LOW}
